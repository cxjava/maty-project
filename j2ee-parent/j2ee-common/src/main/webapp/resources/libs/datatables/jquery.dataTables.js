	...
	var DataTable = function( oInit )
	{
		...
		function _fnAjaxParameters( oSettings )
		{
			...
			if (oSettings.oFeatures.bFilter !== false) {
				aoData.search = oSettings.oPreviousSearch.sSearch;
				aoData.regex = oSettings.oPreviousSearch.bRegex;
			}
			aoData.columnInfo = [];
			aoData.sortInfo = [];
			for (i = 0; i < iColumns; i++) {
				...
				// 列名
				cInfo.columnName = typeof (mDataProp) === "function" ? 'function' : mDataProp;
				/* Filtering */
				if (oSettings.oFeatures.bFilter !== false) {
					cInfo.search = oSettings.aoPreSearchCols[i].sSearch;
			...
			if (oSettings.oFeatures.bSort !== false) {
				...
				aaSort = (oSettings.aaSortingFixed !== null) ? oSettings.aaSortingFixed.concat(oSettings.aaSorting) : oSettings.aaSorting
						.slice();
				...
				for (i = 0; i < aaSort.length; i++) {
					...
					for (j = 0; j < aDataSort.length; j++) {
						var sortInfo = {};
						iCounter++;
						sInfo.columnName = aDataSort[j];
						sInfo.sortDir = aaSort[i][1];
						aoData.sortInfo.push(sortInfo);
					}
				}
				aoData.sortingCols = iCounter;
			}
